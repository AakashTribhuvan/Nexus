...
while True:
    ...
    if intent == "schema":
        print(schema); continue
    elif intent == "invalid":
        print("Invalid question."); continue
    elif intent == "semantic":
        answer = handle_semantic_query(q, schema, get_sample_data())
        print("\n", answer); continue
    ...
    if "INVALID" in validate_sql(sql, schema).upper():
        sql = repair_sql(refined, sql, "Potential logic/syntax error detected.", schema)
        print("Repaired (Validation):", sql)

    result = run_query(sql)
    if "error" in result:
        sql = repair_sql(refined, sql, result['error'], schema)
        print("Repaired (Execution):", sql)
        result = run_query(sql)
